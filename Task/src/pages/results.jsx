        ...
        if (Object.keys(storedAnswers).length > 0) {
            const filtered = products.filter(product => {
                return Object.values(storedAnswers).some(answer => {
                    const titleMatch = product.title && product.title.toLowerCase().includes(answer.toLowerCase());
                    const bodyMatch = product.body_html && product.body_html.toLowerCase().includes(answer.toLowerCase());
                    const tagsMatch = product.tags && product.tags.some(tag => tag.toLowerCase().includes(answer.toLowerCase()));
        ...
        <div>
          <div className="res_home_container">
              <div className="res_image_container">
                  <img src={result_page_img} className="res_top_image" alt="Descriptive Alt Text" />
                  <div className="res_centered_div">
                    <div className="res_title_container">
                        ...
                        <button className='res_start_button'><Link to="/">
            ...
            {filteredProducts.length > 0 ? (
                <Slider {...settings}>
                    {filteredProducts.map(product => {
                        console.log('Logging product:', product); // Log each product
                        return (
